...
void NumberClassifier::extractNumbers(const cv::Mat& src_bgr, std::vector<Armor>& armors) {
    ...
    for (auto& armor : armors) {
        ...
        cv::Point2f dst_quad[4] = {
            ...
            {            0,    top_y},
            {warp_w - 1.0f,    top_y},
            {warp_w - 1.0f, bottom_y}
        };

        cv::Mat warp_gray;
        cv::warpPerspective(
            src_gray, warp_gray, cv::getPerspectiveTransform(src_quad, dst_quad),
            {warp_w, K_WARP_H}, cv::INTER_LINEAR, cv::BORDER_REPLICATE);

        /* ---- 提取数字 ROI（20×28 中央区域） ---- */
        warp_gray = warp_gray(
            cv::Rect((warp_w - K_ROI_SIZE.width) / 2, 0, K_ROI_SIZE.width, K_ROI_SIZE.height));
