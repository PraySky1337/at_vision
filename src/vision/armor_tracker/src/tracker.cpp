...
void Tracker::init(const Armors::SharedPtr& armors_msg) {
    ...
    last_yaw_     = orientationToYaw(tracked_armor.pose.orientation);
...
void Tracker::update(const Armors::SharedPtr& armors_msg) {
    ...
    if (!armors_msg->armors.empty()) {
        ...
        if (min_position_diff < max_match_distance_ && yaw_diff < max_match_yaw_diff_) {
            ...
            // Update EKF
            double raw_yaw  = orientationToYaw(tracked_armor.pose.orientation);
            double pred_yaw = ekf_prediction(6); // 上一步预测

            // unwrap：把 raw_yaw 调整到离 pred_yaw 最近的 2π 周期
            double diff = raw_yaw - pred_yaw;
            if (diff > M_PI)
                raw_yaw -= 2 * M_PI;
            else if (diff < -M_PI)
                raw_yaw += 2 * M_PI;

            double measured_yaw = raw_yaw; // 现在是连续角
