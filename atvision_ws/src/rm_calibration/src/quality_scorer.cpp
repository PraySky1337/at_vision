...
double QualityScorer::scoreIntrinsicFrame(
    const cv::Mat& image, const std::vector<cv::Point2f>& corners, const cv::Size& board_size,
    ...
    double weight_sum = intrinsic_weights_.corner_quality + intrinsic_weights_.board_size
                      + intrinsic_weights_.board_angle + intrinsic_weights_.sharpness;
    if (weight_sum <= 0.0) {
        return 0.0;
    }
...
double QualityScorer::scoreExtrinsicSample(
    const cv::Mat& image, const std::vector<cv::Point2f>& corners, const cv::Mat& rvec,
    const cv::Mat& tvec, const Eigen::Matrix3d& R_imu, const Eigen::Matrix3d* previous_R_imu,
    ...
    double weight_sum = extrinsic_weights_.reprojection_error + extrinsic_weights_.imu_diversity
                      + extrinsic_weights_.board_pose + corner_bonus + sharpness_bonus;
    if (weight_sum <= 0.0) {
        return 0.0;
    }
    ...
    double score = extrinsic_weights_.reprojection_error * reprojection_score
                 ...
                 + extrinsic_weights_.board_pose * board_pose_score + corner_bonus * corner_quality
                 + sharpness_bonus * sharpness_score;
    return clamp01(score / weight_sum);
}

double QualityScorer::calculateCornerQuality(
    const cv::Mat& gray, const std::vector<cv::Point2f>& corners) const {
...
double QualityScorer::calculateBoardSizeScore(
    const std::vector<cv::Point2f>& corners, const cv::Size& image_size) const {
    ...
    double area_ratio =
        static_cast<double>(bounding.area()) / static_cast<double>(image_size.area());
...
double QualityScorer::calculateAngleScore(
    const std::vector<cv::Point2f>& corners, const cv::Size& board_size) const {
...
double QualityScorer::calculateDiversity(
    const std::vector<cv::Point2f>& corners, const std::vector<cv::Point2f>& previous,
...
double QualityScorer::calculateBoardPoseScore(const cv::Mat& rvec) const {
    ...
    cv::Mat R;
    cv::Rodrigues(rvec, R);
    cv::Vec3d normal(R.at<double>(0, 2), R.at<double>(1, 2), R.at<double>(2, 2));
...
double QualityScorer::calculateImuDiversity(
    const Eigen::Matrix3d& R_imu, const Eigen::Matrix3d* previous_R_imu) const {
    if (previous_R_imu == nullptr) {
        // No reference yet; don't penalize early samples.
        return 1.0;
    }

    Eigen::Matrix3d R_rel = previous_R_imu->transpose() * R_imu;
