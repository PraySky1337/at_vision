...
std::vector<Light> Detector::findLights(const cv::Mat& rgb_img, const cv::Mat& bin_img) noexcept {
    using std::vector;
...
void Detector::drawResults(cv::Mat& img) const noexcept {
    ...
    for (const auto& armor : armors_) {
        std::string text = armorTypeToString(armor.type) + " " + armor.classfication_result;

        cv::putText(
            img, text, armor.left_light.top, cv::FONT_HERSHEY_SIMPLEX, 0.8, cv::Scalar(0, 255, 255),
            2);
    }
}
